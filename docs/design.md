# Aether-Realist 设计文档 (架构、安全与性能) - V5.1

本文档深入介绍 Aether-Realist V5.1 版本的核心设计哲学、系统架构以及在安全与性能方面的优化实践。
...
安全是 Aether-Realist 的生命线。V5.1 版本在 V5.0 基础上进一步强化了协议鲁棒性和传输性能。
...
- **协议头加固 (30 Bytes)**:
    - V5.1 头部包含 `Version`、`Timestamp`、`SessionID` 和 `Counter` 字段，全部纳入 AEAD 的 **AAD (附加认证数据)**。
    - **设计优势**: 攻击者无法在不破坏加密验证的情况下篡改任何头部字段。
- **抗重放攻击 (Anti-Replay)**:
    - **时间戳校验**: 服务端强制 Record 时间戳与本地时间偏差在 ±30s 内。
    - **计数器单调性检查**: 5.x 采用严格的 **Counter 递增检查**，替代 V4 的 IV 去重缓存，提供确定性的抗重放保证。
...
- **流量混淆 (Traffic Obfuscation)**: 
    - **握手阶段**: 强制 Metadata Padding。
    - **传输阶段**: V5.1 移除 Data Padding，采用 **64KB 动态切片** (应用层 Record)，底层层实现依赖 QUIC 协议栈根据 MTU 自动分包。
...
### 3.1 HTTP/3 (QUIC) 优势
- **0-RTT 建连**: V5.1 配合计数器机制，**正式开启 0-RTT**，实现真正的高性能握手恢复。
...
- **超大流控窗口**: V5.1 引入分级窗口配置 (`WINDOW_PROFILE`)，默认 `Normal` 模式下 Stream/Conn 窗口为 2MB/3MB，`Aggressive` 模式下可达 4MB/6MB，但严格控制单流不超过 4MB 以隐藏特征。
- **加速爬升 (Fast Ramp-up)**: 初始窗口 (Initial Window) 优化。
